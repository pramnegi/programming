...
struct Node *check_lca(struct Node* root, int n1, int n2)
{
    if (root == NULL)
        return NULL;
    ...
    Node *left = check_lca(root->left, n1, n2);
    Node *right = check_lca(root->right, n1, n2);
...
int main()
{
    ...
    while(ele1 || ele2) {
        ...
        if(check_element(root,ele1) && check_element(root,ele2))
            cout  <<  "LCA OF\t" << ele1 << "and\t"  << ele2 << "is" <<  "\t" << check_lca(root, ele1, ele2)->key;
        else
